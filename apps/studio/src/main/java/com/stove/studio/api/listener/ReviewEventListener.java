 ...
 *
 * <p>트랜잭션과 멱등 마킹은 {@link StudioService} 가 소유한다.
 */
@Slf4j
@Component
...
public class ReviewEventListener {
    ...
    @KafkaListener(topics = Topics.REVIEW, groupId = StudioService.CONSUMER_GROUP)
    public void onReviewEvent(ConsumerRecord<String, String> record) {
        ...
        if (envelope.isType(EventType.REVIEW_APPROVED)) {
            ...
            studioService.applyApproval(envelope.eventId(), envelope.eventType(),
            ...
            studioService.applyRejection(envelope.eventId(), envelope.eventType(),

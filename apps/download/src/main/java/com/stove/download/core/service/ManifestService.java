 ...
 *
 * <p>문서 ID 가 {@code productCode:releaseId} 라 {@code ReleasePublished} 를 몇 번 받아도 같은 문서를
...
public class ManifestService {
    ...
    /** studio → ReleasePublished : 공개된 패치 매니페스트만 등록 */
    public void register(ReleasePublishedEvent event) {
        manifestRepository.save(PatchManifest.from(event));
        log.info("매니페스트 등록 productCode={} releaseId={} version={}",
                event.productCode(), event.releaseId(), event.productVersion());
    }

    /** 패치 이력. 최신순이며 보유 여부와 무관하게 열려 있다. */
    public List<PatchManifest> history(String productCode) {
        return manifestRepository.findByProductCodeAndReleaseIdIsNotNullOrderByReleasedAtDesc(productCode);
    }
    ...
    public PatchManifest requireRelease(String productCode, Long releaseId) {
        return manifestRepository.findByProductCodeAndReleaseId(productCode, releaseId)
                .orElseThrow(() -> new BusinessException(ErrorCode.UPSTREAM_UNAVAILABLE,
                        "릴리스 매니페스트 동기화 중입니다. productCode=" + productCode));
    }
}

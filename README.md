...
스마일게이트 STOVE 스토어 서버 포지션을 겨냥한 Java 21 + Spring Boot 3.5 마이크로서비스 모노레포다.
크리에이터 입점부터 판매·지급·정산까지 9개 서비스를 EDA 로 연결했다.
Saga, Transactional Outbox, 멱등성, 검증 게이트를 실제 동작하는 코드로 구현한다.
...
저장소는 도메인 성격을 보고 갈랐다. 트랜잭션과 정합성이 중요한 도메인은 MySQL,
스키마가 유동적인 패치 매니페스트는 MongoDB, 검색 트래픽은 Elasticsearch 다.
...
토픽은 애그리거트 단위(`stove.order.v1` 등)로 두고 메시지 키는 주문번호나 상품코드를 쓴다.
그래서 같은 애그리거트의 순서가 보장된다.

## 3. 설계 과제와 해법

| 과제 | 해법 | 코드 |
|---|---|---|
| 이벤트 유실 (DB 커밋 ↔ Kafka 발행 원자성) | Transactional Outbox. 비즈니스 변경과 같은 트랜잭션에 이벤트를 적재하고 폴링 릴레이가 발행한다 | `common/messaging/outbox` |
| 중복 수신 (재전송·리밸런싱) | Inbox 멱등 가드. `(event_id, consumer_group)` 유니크를 처리와 같은 트랜잭션에서 마킹한다 | `common/messaging/inbox` |
| 결제 성공 후 지급 실패 | Saga 보상 트랜잭션. `LicenseIssueFailed` 가 payment 자동 환불을 부른다 | `license/…/PaymentEventListener` |
| 금액 위·변조 | 검증 게이트를 4단계로 나눠 배치했다 (아래) | |
...
| license 장애가 다운로드 장애로 전이 | 동기 호출 대신 권한 사본을 이벤트로 들고 있는다. license 를 정지시키고 확인했다(다운로드 20/20, 9.3ms) | `download/Entitlement` |
| 정산 중복 집계(금전 사고) | Inbox + `(order_no, product_id, record_type)` 유니크 이중 방어 | `settlement_record` |
| 환불 시 정산 역산 | 자기 원장의 SALE 레코드를 부호 반전해 상계한다. 다른 서비스에 되묻지 않는다 | `SettlementRecordService#recordRefund` |
| 경계가 시간이 지나며 흐려짐 | ArchUnit 으로 계층과 의존 방향을 테스트로 강제한다. 규칙을 처음 돌렸을 때 위반이 166건이었다 | `common/archunit` |
| 비동기 발행이 분산 추적을 끊음 | 추적 컨텍스트를 이벤트와 같은 트랜잭션에 저장했다가 발행 시점에 되살린다. 자동 계측은 릴레이 스케줄러의 컨텍스트를 싣기 때문이다 | `common/messaging/trace` |
| 재시도를 소진한 메시지가 사라짐 | DLT 와 재투입 API 로 파티션을 막지 않으면서 유실만 없앤다. Outbox `DEAD` 도 HTTP 로 회수한다 | `common/kafka`, `common/messaging/ops` |

### 검증 게이트 4단계

1. 주문 시점: 클라이언트 금액을 믿지 않고 catalog 가격으로 다시 계산한다 (`PlaceOrderService`)
2. PG 사전등록: 승인 전에 서버가 결제 금액을 PG 에 먼저 등록한다 (`Payment#prepare`)
3. 콜백 대조: PG 승인 금액 ≠ 사전등록 금액이면 승인을 거부한다 (`Payment#approve`)
4. 멱등키: 중복 콜백은 상태와 `idempotency_key` 유니크 제약으로 흡수한다

### 멱등성 전략 (서비스 성격에 맞춰 다르게)

| 방식 | 적용 서비스 | 이유 |
|---|---|---|
| Inbox 테이블 + 도메인 유니크 | order, payment, license, settlement, studio, review, catalog | 상태 전이와 금전 처리라 "정확히 한 번"이 필요 |
...
전체 스택(인프라 10 + 앱 10)은 6.1 GiB 이상을 쓰므로 8GB 랩탑에서는 산술적으로 안 뜬다.
...
컨테이너 라이브러리가 클래스패스에 없어 임포트가 컴파일되지 않기 때문이다.
...
첫 사용 전에 한 번만 실행한다 (머신별 값이라 리포에 두지 않는다, 10번):
...
`docker-in-docker` 를 쓰므로 컨테이너가 자기 Docker 데몬을 갖는다. 덕분에 Testcontainers 테스트와
...
빌려 쓰는 머신처럼 brew·node·JDK 설치가 여의치 않은 환경을 위한 진입점이다.
...
그 가정이 싫으면 A 를 쓴다. 애플리케이션 스택 실행은 호스트에서 `docker compose` 로 한다.
...
| kafka-exporter | http://localhost:9308/metrics 에서 컨슈머 랙을 본다. 앱 지표로는 판정할 수 없다([D-026](docs/defects.md#d-026)) |
| Tempo (추적 수집) | OTLP/HTTP `localhost:4318`, 조회 API `localhost:3200` |
| Grafana | http://localhost:3000 (anonymous). Prometheus 와 Tempo 데이터소스가 미리 등록돼 있다 |
| Actuator | `http://localhost:808X/actuator/health`, `/actuator/prometheus` |

주문 하나를 9개 서비스에 걸쳐 따라가려면 Grafana → Explore → Tempo → Search 로 간다.
...
자동 계측만으로는 끊긴다. 적재 시점에 붙잡은 컨텍스트를 릴레이가 되살려서 잇는다
([decisions.md](docs/decisions.md) 17번).

원격 전체 스택에서 확인한 결제 콜백 1건의 실제 스팬 트리다. 6개 서비스를 지나고 Kafka 를 2회 통과한다.
...
세 컨슈머의 부모가 릴레이 스케줄러가 아니라 원래 HTTP 요청 스팬이라는 점이 요점이다.
...
> 정작 요점인 스팬 트리의 모양은 그대로다. 부모가 릴레이가 아니라 HTTP 요청 스팬이라는 것 말이다.
...
의도적으로 거절되는 요청이다. 스켈레톤이 어디를 막는지 보여준다.
...
> `stove.outbox.dead` 와 `stove.kafka.dead-lettered` 에 Prometheus 알람이 걸려 있다
> (`infra/prometheus/alerts.yml`).
>
> 받는 쪽 적체에도 알람이 있다. `ConsumerLagGrowing` 과 `ConsumerStalled` 다.
> 이쪽 지표만 출처가 앱이 아니라 `kafka-exporter` 다. 앱이 내는 랙 지표는
> 컨슈머가 멈추면 같이 멈춰서 알람이 가장 필요한 상태에서 침묵한다([D-026](docs/defects.md#d-026)).

## 6. 외부 연동은 포트로 분리

실연동 대상은 인터페이스(포트)로 두고 구현이 없는 것만 스텁을 쓴다. 도메인 규칙이 외부 사정에 오염되지 않게 하려는 것이다.
다섯 포트 중 둘은 실제 어댑터가 이미 있고 설정으로 고른다.

| 포트 | 스텁 | 실제 어댑터 | 고르는 법 |
|---|---|---|---|
| `BuildStorage` (studio) | `MockBuildStorage` | `S3BuildStorage` | `stove.storage.provider` = `mock`(기본) / `s3` |
| `DownloadUrlSigner` (download) | 없음 | `CdnUrlSigner`(HMAC 실서명), `S3PresignedUrlSigner` | `stove.download.url-strategy` = `cdn`(기본) / `s3` |
| `PgClient` (payment) | `MockPgClient` | 없음 (PG사 / 스토브캐시) | 해당 없음 |
| `RatingBoardClient` (review) | `MockRatingBoardClient` | 없음 (게임물관리위원회 접수) | 해당 없음 |
| `TaxInvoiceIssuer` (settlement) | `MockTaxInvoiceIssuer` | 없음 (전자세금계산서) | 해당 없음 |

스텁은 전부 `@Profile("!prod")` 라 운영에서 조용히 도는 일이 없다. 그 조건은 ArchUnit 이 검사한다
...
- ~~Kafka DLT + 재처리 운영툴, Outbox `DEAD` 레코드 알람~~ → 했다.
  적용하다 store 와 download 에는 재시도 정책조차 없었다는 것이 드러나 컨슈머 정책을 `common:kafka` 로
  분리했다. 이제 9개 서비스가 같은 실패 처리를 갖는다 ([decisions.md](docs/decisions.md) 19번)
- ~~전 구간 시나리오 관통(등록→심의→구매→지급→정산)~~ → 했다. 그리고 CI 의 판정 조건이 됐다.
  셸 스모크였던 것을 `:e2e` 모듈로 옮겼다. 트랙 A~C·환불·결제 거절을 게이트웨이 경유로
  42건 관통하고 거기에 관측 4건(트레이스 연결·적체 수렴·DLT·종단 지연)이 얹혀 main push 에서 돈다.
  배포 게이트(컨테이너·인프라·라우팅 차단)는 성질이 달라
...
- ~~분산 추적(Micrometer Tracing + OTLP)으로 correlationId 를 traceId 로 승격~~ → 했다.
  ...
  때문이었다. 자동 계측은 `send()` 를 부른 스레드(릴레이 스케줄러)의 컨텍스트를 싣는다.

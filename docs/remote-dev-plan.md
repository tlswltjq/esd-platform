...
#### 개발 머신이 둘이 되면서 드러난 것 (2026-08-16)

개발을 **실습실 i5(32GB, 로컬 실행)** 와 **랩탑 M1(8GB, 원격 실행)** 두 곳에서 하게 되면서
Phase 6 의 가정 하나가 깨졌다 — **원격 작업 디렉터리가 하나였다.**

| | 문제 | 고친 방식 |
|---|---|---|
| **작업 디렉터리** | 기본값이 `/opt/stove-stack` 고정. 두 머신이 같은 자리에 rsync 하면 `--delete` 가 앞의 작업본을 **조용히 지운다** | 기본값을 `~/stove/<사용자>-<호스트>` 로. **홈 아래**인 이유는 `/opt` 이 root 소유라 머신을 붙일 때마다 sudo 가 필요해서다 |
| **측정 산출물** | `runs/` 가 로컬에 없어 `--delete` 대상이었다. 회차 하나가 40분이다 | `--exclude 'runs/'` |
| **배포 경로** | `--build` 가 Gradle 을 안 부른다. 원격에 남은 옛 jar 가 그대로 이미지가 됐다 | `stack up` 이 `bootJar` 를 먼저 부른다 |
| **점유** | CI 러너가 **같은 박스의 같은 컨테이너**를 쓴다. 실제로 두 번 부딪혔다 | `stack up|down` 이 CI·측정 실행 중이면 멈춘다 |

**전체 스택은 원격 한 대에 한 벌이 상한이다.** 인프라 11종이 `container_name:` 으로 이름을
고정하기 때문이다(포트는 `docker-compose.ci.yml` 이 이미 `!reset` 한다 — **이름만 남았다**).
Phase 7 에서 배포 스택을 상주시키면 그 한 벌을 배포가 차지하므로, **`container_name:` 제거가
CD 의 선행 조건**이 된다. 그때까지는 e2e·chaos 가 배포 스택을 밀어낸다.

> 점유 검사를 처음 쓴 판은 **아무것도 안 도는데 넷 다 "실행 중" 이라고 했다.**
> `pgrep -f` 가 패턴 문자열을 명령줄에 담고 있는 **자기 자신을 잡은 것**이다.
> PID 로 거르는 것으로는 부족했다 — 명령 치환이 포크한 자식은 명령줄이 같고 PID 만 다르다.
> 프로브에만 있는 표식을 심어 걸렀다. **가드는 만든 뒤에 대조군으로 확인해야 한다**(D-021·D-023).

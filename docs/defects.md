...
| [D-033](#d-033) | 인수 테스트가 데이터가 적을 때만 통과 — 21회차부터 영구 실패 | 게이트 마비 | **수정됨** |
| [D-034](#d-034) | 게이트웨이만 graceful shutdown 이 없어 배포마다 요청이 끊김 | 사용자 영향 | **수정됨** |
| [D-035](#d-035) | 컨테이너 종료 유예가 짧아 graceful shutdown 이 SIGKILL 로 잘림 | 사용자 영향 | **수정됨** |
| [D-036](#d-036) | 통합 테스트 컨텍스트가 늘면 관계없는 기존 클래스가 빨개짐 | 게이트 마비 | **수정됨** |
| [D-037](#d-037) | 장애를 주입했는데 컨슈머 그룹 멤버가 둘이라 파티션만 옆으로 옮겨 감 | 게이트 마비 | **수정됨** |
...
블로킹 재시도가 파티션을 건당 7.8초씩 멈춰 세웠기 때문인데, 그 정지를 앱 지표로는 볼 수 없다 —
...
나머지는 동시에 보낼 수 있게 되어 배치 직렬 대기가 사라졌다 —
배치 처리 0.47s → 0.28s, 릴레이 처리량 138.0 → 156.1 events/s(**×1.13**).

**여기까지가 이 수정의 몫이다.** 간판 수치 `138 → 480.5 events/s (×3.48)` 는
적체가 남았는데도 고정 1초를 쉬던 폴링을 걷어낸 **별개 수정과 합친 값**이다.
그쪽은 순서와 무관한 스케줄러 유휴 문제다 — 두 몫을 가른 표는
[performance.md](performance.md) 6장에 있다.
...
멈추므로 레코드 하나가 회전 한 번을 통째로 쓴다 — 대략 `장애 지속시간 ÷ 회전 주기` 다.

회전 주기는 `백오프(1+2+4=7초) + 4회 × 시도당 지연` 이고, 시도당 지연이 장애 종류로 갈린다.
[chaos.md](chaos.md) 8-6 에서 지속시간을 바꿔 가며 쟀다 — **테이블 차단 7.8초 · 스키마 차단 19.8초**
이고, 차이는 전부 `spring.datasource.hikari.connection-timeout`(3초) × 4회 시도에서 나온다.
**처음 이 자리에 적었던 "7초" 는 백오프만 센 값이라 틀렸다.**
...
> **세어 보고 남기기로 했다**([#46](https://github.com/tlswltjq/stove/issues/46)). 그 이슈가 "결론을 내기 전에 세는 것이 먼저다" 로
> 걸어 둔 선행 조건을 실측으로 채웠다 — license outbox **4,153건 중 `LicenseIssueFailed` 0건**
> (전부 `LicenseIssued` 4,113 + `LicenseRevoked` 40). chaos 2회차도 같은 방향이다: 저장소 장애
> 800건에서 **보류 47 · 환불 0**.
>
> **추론이 맞았다. 그런데 지우지는 않는다.** 0 은 "필요 없다" 가 아니라
> **"그 실패가 아직 안 났다"** 는 뜻이고, 지금 지우면 계약(`LicenseIssueFailedEvent`)과 소비 경로가
> 함께 사라져 **비저장소 실패가 처음 생기는 날 되돌릴 수단이 없는 채로 그 사실을 알게 된다.**
>
> 대신 이슈의 3번("남긴다면 무엇이 살아 있음을 보장하는가")에 답했다 —
> `stove.payment.compensated` 카운터가 "몇 번 지나갔나" 를 세고(0 도 값이다),
> `PaymentCompensationTest` 가 실 MySQL 위에서 그 경로를 지난다.
> 예전에는 단위 테스트뿐이라 **배선이 끊겨도 아무도 몰랐다**(D-030 과 같은 자리).
...
> **닫았다** — `OrderStatus.EXPIRED` 와 `OrderExpirySweeper` 를 넣었다([#43](https://github.com/tlswltjq/stove/issues/43)).
> 이 항목이 만료 스케줄러를 한 번 버렸던 이유(payment 가 `OrderCanceled` 를 듣지 않아 결제 대기
> 레코드가 열린 채 남는다)는 payment 쪽 창이 닫히면서 해소됐다. 다만 **"아무도 반응하지 않는
> 스케줄러를 넣지 않는다" 는 판단은 그대로 지켰다** — 만료는 이벤트를 내지 않는다.
>
> 실측으로 드러난 크기는 예상과 달랐다: `CREATED` 98,750건 중 **98,746건이 하루(2026-08-04)에
> 만들어진 부하 시험 잔재**였고 실제 유입은 하루 1~3건이다. **쌓이는 속도가 아니라 쌓이는
> 구조가 문제였다** — 그리고 그 구조는 유입이 적어도 그대로다.
...
## D-033 인수 테스트가 데이터가 적을 때만 통과했다

**상태** 수정됨
**영향** 게이트 마비 — `main` 이 8/15부터 계속 빨감
**위치** `e2e/.../TrackACreatorFlowTest#appearsInSearchIndex`
**재현** e2e 를 21회 이상 돌린 스택에서 인수 시나리오를 돌린다 — 그 한 건이 반드시 실패한다
**실측** 응답 20건(`productId` 15~34 오름차순), 이번 회차 상품은 **41번**

### 무슨 일이

`main` 의 e2e 가 46건 중 **1건**만 실패하기 시작했다.

```
트랙 A — 등록 → 심의 → 노출 > store: 검색 색인에 반영된다 (ProductChanged 관통) FAILED
  60초 안에 성립하지 않았다. 마지막 응답: {"success":true,"data":[{"productId":15,...
```

**먼저 의심한 것(이벤트가 안 건너간다)은 아니었다.** store 는 제때 색인했다.

```
19:08:46.520  StoreService  색인 동기화 productCode=GAME-E2E-1786874923 status=APPROVED
19:08:46.603  StoreService  색인 동기화 productCode=GAME-E2E-1786874923 status=ON_SALE
```

테스트가 실패한 것은 그로부터 **1분 뒤**다. 이벤트도 컨슈머도 ES 도 전부 정상이었다.

### 원인 — 페이지 1에 자리가 없다

테스트는 `q=인수` 로 묻고 응답에서 자기 `productCode` 를 찾았다. **페이지를 지정하지 않는다.**

```
반환 건수: 20                       ← 기본 size
productId: [15, 16, ... , 33, 34]   ← 오름차순
이번 회차 상품: 41
```

**15~34 는 전부 지난 회차들이 남긴 "인수 시나리오 게임" 이다.** e2e 가 돌 때마다
자기 검색 결과를 한 칸씩 밀어냈고, 21번째 회차부터 자기 상품이 페이지 1 밖으로 나갔다.

| | |
|---|---|
| 페이지 1을 채운 상품 | 34번 — **2026-08-13 17:05** |
| 그다음 회차부터 구조적 실패 | 35번 — 2026-08-13 17:15 |

**통과하던 이유가 "색인이 됐다" 가 아니라 "아직 20건이 안 찼다" 였다.**

### 규칙은 이미 있었는데 한 필드에만 적용되지 않았다

`Journey` 가 그 원칙을 이미 적어 두고 있었다.
...
`productCode`·`memberId`·`pgTxId` 에는 스탬프가 붙어 있었다. **제목에만 없었다.**
그리고 검색은 이름만 본다(`findByStatusAndNameContaining` — `productCode` 는 대상이 아니다).
**하필 검색 테스트가 묻는 유일한 필드가 스탬프 없는 그 필드였다.**

### 수정

제목에도 스탬프를 박고, 이번 회차의 스탬프로 묻는다.
...
결과가 한 건으로 좁혀지므로 **"무언가 검색된다" 가 아니라 "방금 만든 그것이 색인에 갔는가"**
를 묻게 되고, 쌓인 데이터에 영향받지 않는다.

### 옆 테스트는 살아남았다 — 왜인지가 답이었다

같은 클래스의 `appearsInOnSaleList`(7번)는 계속 통과했다.

```java
() -> Stove.gateway.get("/api/v1/products?sort=productId,desc")
```

**`desc` 로 최신을 앞에 오게 했기 때문이다.** 같은 함정 위에 있었는데 한쪽에만 보호가 있었다.
다만 그쪽도 근본 해법은 아니다 — 페이지 1에 의존하는 것은 같고 방향만 유리하다.

### 무엇이 이 부류인가

[D-021](#d-021)(규칙이 대상 0건을 검사) · [D-023](#d-023)(게이트가 컨테이너 0개에도 통과) ·
[D-031](#d-031)(알람이 조건이 참인 내내 안 울림)과 같은 자리다.
넷 다 **장치의 판정이 검증 대상이 아니라 주변 상태에 달려 있었다.**
앞의 셋은 그래서 조용히 통과했고, 이것은 반대로 **조용히 영원히 실패했다** —
방향만 다르고 원인은 같다.

### 남은 것

`/api/v1/storefront/products` 응답에 **페이지 정보가 없다**(`{"success":true,"data":[...]}` 뿐).
전체 건수도 다음 페이지 유무도 없으므로 **20건이 "전부" 인지 "첫 장" 인지 응답만 봐서는 모른다.**
테스트가 그걸 구분하지 못한 것은 테스트만의 잘못이 아니다. 이번 수정은 테스트 쪽만 닫았다.

---

## D-034 게이트웨이만 graceful shutdown 이 없다

**상태** 수정됨
**영향** 사용자 영향 — 재배포마다 진행 중이던 요청이 끊긴다
**위치** `apps/gateway/src/main/resources/application.yml`
**재현** `DeploymentShutdownContractTest#everyAppShutsDownGracefully`
**실측** 앱 10종 중 9종이 `server.shutdown: graceful`, **gateway 만 미설정**

### 무슨 일이

중단·복구 시나리오([resilience-scenarios.md](resilience-scenarios.md) R-01)를 붙이면서
앱 전부의 종료 설정을 한 자리에서 훑었더니 하나가 비어 있었다.

```
Expecting empty but was: ["gateway (server.shutdown=null)"]
```

**하필 모든 트래픽이 지나는 문이다.** `Stove` 클래스 주석이 적어 둔 대로
"거의 모든 호출이 게이트웨이를 거친다 — 실제 경로가 그렇기 때문이다.
PG 콜백도 클라이언트 요청도 창작자의 스튜디오 호출도 밖에서 온다."

그래서 이 한 줄이 빠진 대가는 **아홉 서비스가 켜 둔 graceful 을 전부 무의미하게 만드는 것**이다.
하류가 아무리 요청을 마치고 닫아도, 그 응답을 사용자에게 전달할 문이 이미 닫혀 있다.

### 왜 아무도 못 봤나

**게이트웨이만 설정 파일의 모양이 다르다.** 나머지 아홉은 DB·Kafka·Outbox 설정이 줄줄이 있는데
게이트웨이는 라우팅 표가 대부분이라(`docker-compose.apps.yml` 에서 혼자 `*common-env` 를 쓰지 않는
서비스이기도 하다) **"이 서비스에는 원래 없는 설정" 처럼 읽힌다.**

그리고 이 결함은 **정상 경로에서 절대 보이지 않는다.** 배포 순간에만 나타나고,
그 순간에 끊긴 요청은 로그에도 지표에도 남지 않는다.
...
계약을 테스트로 옮겼다 — 앱이 하나 늘면 그 앱도 자동으로 검사 대상이 된다.
**규칙을 지키는 데 사람의 성실함이 필요하면 그 규칙은 지켜지지 않는다**([chaos.md](chaos.md) 9장 7번).

---

## D-035 graceful shutdown 이 SIGKILL 로 잘린다 — 앱 10종 전부

**상태** 수정됨
**영향** 사용자 영향 — **설정은 있는데 동작하지 않는다**
**위치** `docker-compose.apps.yml` + 앱 10종의 `application.yml`
**재현** `DeploymentShutdownContractTest#containerGraceOutlastsSpringShutdownPhase`
**실측** 컨테이너 유예 **10초**(도커 기본값) ≤ 스프링 종료 단계 **30초**(스프링 기본값) — 10종 전부

### 무슨 일이

D-034 를 고치고 나면 "이제 열 개 다 graceful 이다" 로 끝날 자리다. 그런데 한 겹 더 있었다.

| | 값 | 어디서 오나 |
|---|---:|---|
| 컨테이너 종료 유예 | **10초** | 도커 기본값 — `stop_grace_period` 를 아무도 안 적었다 |
| 스프링 종료 단계 타임아웃 | **30초** | 스프링 기본값 — `timeout-per-shutdown-phase` 를 아무도 안 적었다 |

도커는 SIGTERM 을 보내고 유예가 지나면 **SIGKILL** 을 보낸다. 스프링은 진행 중인 요청을
최대 30초까지 기다릴 작정인데 도커가 10초에 프로세스를 죽인다.
**둘 다 기본값으로 두면 이 부등호는 항상 거짓이다** — 즉 graceful 은 처음부터 20초를 잃은 채였다.

### 왜 이게 D-034 보다 나쁜가

D-034 는 설정이 **없다**. 훑으면 보인다.
이것은 설정이 **있다** — `server.shutdown: graceful` 이 열 파일에 또박또박 적혀 있다.

> **"장치가 있다" 와 "장치가 그 자리를 지킨다" 는 다르다.**
> [D-021](#d-021)(규칙이 대상 0건을 검사) · [D-023](#d-023)(게이트가 컨테이너 0개에도 통과) ·
> [D-031](#d-031)(알람이 조건이 참인 내내 안 울림)과 같은 부류다.
> 넷 다 **장치를 켠 것으로 확인을 대신했다.**

그리고 이 결함은 **한 파일만 봐서는 존재를 알 수 없다.** compose 에는 종료 유예가 안 적혀 있고
`application.yml` 에는 graceful 이 적혀 있다 — 둘 다 각자로는 이상해 보이지 않는다.
어긋남은 두 파일 *사이*에만 있다.

### 수정

양쪽을 명시하고, 그 부등호를 테스트가 지킨다.

```yaml
# docker-compose.apps.yml — x-app 앵커
stop_grace_period: 40s

# 앱 10종의 application.yml
spring:
  lifecycle:
    timeout-per-shutdown-phase: 30s
```

40 > 30 이고, 여유 10초는 종료 단계가 끝난 뒤 JVM 이 실제로 내려가는 데 쓴다.
**값을 고르는 것보다 부등호를 테스트에 박는 것이 중요하다** — 나중에 누가
`timeout-per-shutdown-phase` 를 늘리면서 compose 를 안 고치면 그 순간 다시 잘리는데,
지금은 그게 빨개진다.

---

## D-036 통합 테스트에 클래스를 하나 더하자 관계없는 기존 클래스가 빨개졌다

**상태** 수정됨
**영향** 게이트 마비 — 원인과 증상이 다른 파일에 있다
**위치** 루트 `build.gradle` 의 `integrationTest` 태스크
**재현** payment 통합 소스셋에 프로퍼티 조합이 다른 `@SpringBootTest` 를 하나 더한다
**실측** `StrandedRefundResumeTest` **6건 중 5건** 실패 — 원인은 그 클래스가 아니다

### 무슨 일이

R-04 시나리오 테스트(`RefundSweeperWiringTest`) 한 클래스를 넣고 원격에서 돌렸더니,
**넣지 않은 클래스**가 빨개졌다.

```
Caused by: BeanCreationException: Error creating bean with name 'flywayInitializer' ...
  Unable to obtain connection from database: Too many connections
```

### 원인 — 운영 풀 크기를 테스트 JVM 이 컨텍스트 수만큼 곱해 쓴다

셋이 겹쳤다.

1. 앱의 `maximum-pool-size: 20` 은 **운영 인스턴스 한 대**를 위한 값이다.
2. HikariCP 의 `minimumIdle` 기본값은 `maximumPoolSize` 다 — **놀아도 20개가 열려 있다.**
3. 스프링은 `@SpringBootTest(properties = ...)` 의 조합마다 컨텍스트를 따로 만들고
   **JVM 이 끝날 때까지 캐시에 들고 있다.** 컨텍스트마다 풀이 하나씩 붙는다.

MySQL 기본 `max_connections` 는 151 이므로 **컨텍스트 여덟 벌이면 벽이다.**
payment 는 통합 테스트 클래스가 13개였고 이미 그 언저리에 있었다 —
**한 벌을 더할 자리가 남아 있지 않았다.**

빨개지는 것이 새로 넣은 클래스가 아니라는 점이 이 결함의 성질이다.
먼저 뜬 컨텍스트가 커넥션을 다 쥐고, **나중에 뜨는 쪽이 실패한다.**
그 순서는 실행 순서에 달려 있어 회차마다 다른 클래스가 빨개질 수 있다.
...
**시스템 프로퍼티로 덮는 것이 요점이다.** `@SpringBootTest(properties = ...)` 로 덮으면
그 자체가 컨텍스트 캐시 키가 되어 **컨텍스트를 한 벌 더 만든다** — 줄이려던 것을 늘리는 셈이다.
시스템 프로퍼티는 키에 들어가지 않으므로 캐시 재사용을 그대로 두고 풀만 줄인다.

### 남은 것

이 수정은 **한계선을 20에서 5로 미룬 것이지 없앤 것이 아니다.** 컨텍스트가 서른 벌을 넘기면
같은 벽에 다시 닿는다. 그때는 프로퍼티 조합을 줄이는 쪽(공유 기반 클래스)이 답이지
풀을 더 줄이는 쪽이 아니다 — **풀이 작아지면 이번에는 풀 고갈이 간헐 실패로 나타난다.**

그리고 **e2e 가 남긴 데이터는 아무도 지우지 않는다**(상품 41건, ES 38건, 8/10부터 누적).
"e2e 가 남긴 것이 다음 e2e 의 입력이 된다" 는 것은 이 항목이 처음 드러낸 것이 아니라
`Journey` 의 STAMP 주석이 이미 알고 있던 것이다 — **알고 있는 것과 전부에 적용하는 것은 다르다.**

---

## D-037 장애를 주입했는데 파티션만 옆으로 옮겨 갔다 — 컨슈머 그룹 멤버가 둘이었다
...
R-03(`ConsumerRestartCatchUpTest`)이 원격에서는 통과하고 **CI 에서 빨개졌다.**

```
ConsumerRestartCatchUpTest > 따라잡기가 이중 지급이 되지 않는다 FAILED
  java.lang.AssertionError: 컨슈머를 멈췄는데 지급이 일어났다
```

멈추는 데 실패한 것이 아니다. **자기 컨텍스트의 리스너는 정확히 멈췄다.**

### 원인 — 멈춘 것이 그룹의 전부가 아니었다

license 통합 소스셋에는 컨텍스트가 두 벌 있다.
...
종료 로그가 그대로 남겼다. 파티션을 쥔 것은 R-03 이 멈춘 컨슈머가 아니라 옆 컨텍스트다.

```
[Consumer clientId=consumer-license-1, groupId=license] Revoke previously assigned partitions stove.payment.v1-0
```

`consumer-license-1` 은 톰캣을 띄운 쪽(`RANDOM_PORT`)이고, R-03 의 컨텍스트는
정지·재기동을 반복해 그때마다 클라이언트 id 가 올라간 `consumer-license-4` 였다.

### 왜 원격에서는 통과했나

**파티션이 어느 멤버에게 가는지는 그룹 합류 순서에 달려 있다.** 회차마다 다르고,
R-03 의 컨텍스트가 파티션을 계속 쥐고 있으면 옆 컨텍스트는 아무것도 안 받으므로
같은 코드가 초록으로 끝난다. **간헐 실패이고, 초록이 아무것도 보증하지 않는다.**

### 이 결함의 성질 — 실패한 쪽이 옳았다

이 항목이 D-021 · D-023 · D-031 · D-035 와 갈리는 지점이다. 그쪽은 전부
**장치가 있는데 안 도는 것을 아무도 몰랐다.** 여기서는 하네스가 제대로 잡았다 —
`awaitStaysUnissued` 가 "중단 중 미지급" 을 먼저 묻지 않았다면, 뒤의 "재기동하니
따라잡았다" 는 **한 번도 멈춘 적 없는 컨슈머가 그냥 소비한 것**을 그렇게 읽었을 것이다.

> 장애 실험의 실패 모드는 "장애가 안 났다" 가 아니라 **"장애가 안 났는데 났다고 믿는다"** 다
> (`scripts/chaos/README.md`)

**주입 확인이 본체다** 를 넣어 둔 값이 여기서 나왔다. 그것이 없었으면 이 PR 은
초록으로 병합되고, R-03 은 아무것도 재지 않는 테스트로 남았을 것이다.
...
폴링을 재운 것과 **같은 처방이다** — 빈과 엔드포인트 등록은 그대로라 배선 검증은 유지되고,
그룹에만 안 붙는다. `*ContextTest` 는 구성이 **올라오는지**를 묻지 **도는지**를 묻지 않으므로
재워서 잃는 판정이 없다.

### 남은 것

**같은 함정을 세 번째로 밟았다.** 캐시된 컨텍스트가 쥐는 것이 배경 스레드(릴레이 폴링) ·
커넥션 풀([D-036](#d-036)) · 그리고 이번의 그룹 멤버십이다. 앞의 둘은 **자원**이라 고갈로
나타났지만, 멤버십은 고갈되지 않고 **옮겨 간다** — 그래서 증상이 실패가 아니라
"정상 동작" 의 모습을 한다.

지금 이 저장소에서 리스너를 멈춰 장애를 만드는 테스트는 R-03 하나뿐이라 노출 지점도 하나다.
**둘째가 생기면 그때는 클래스마다 막을 것이 아니라** 그룹 멤버가 하나임을 회차가 보장해야 한다
(공유 기반 클래스로 컨텍스트 조합을 줄이는 쪽이 D-036 의 다음 수와 같은 답이다).

...
**재현** `PaymentWindowTest` (통합 소스셋) 4건
...
**사전등록 이후의 창은 열려 있다.** `PENDING` 인 채로 오래 둔 결제는 지금도 승인된다 —
그쪽을 닫으려면 PG 세션 만료와 맞물려야 하고, 콜백을 거절하는 순간
"PG 에는 승인, 우리는 거절" 이 생긴다. 창을 하나 더 두려면 그 처리를 먼저 정해야 한다.

주문 쪽 상태는 여전히 `CREATED` 로 남는다. 금전 경로는 닫혔으므로 정합성 문제는 아니지만,
"영원히 결제 대기인 주문" 이 쌓인다.
...
[PR #35](https://github.com/tlswltjq/stove/pull/35) 가 D-026(컨슈머 랙 지표가 컨슈머와 함께
멈춘다)을 쓰고 있어 번호를 비워 두었다. 그 PR 이 병합되지 않으면 결번으로 남는다.

...
**재현** `PaymentWindowTest` · `PaymentCheckoutWindowTest` (통합 소스셋) 9건
...
**`CANCELING` 에서 멈춘 건을 자동으로 재시도하는 장치가 없다.** PG 환불 호출과 확정 커밋
사이에서 죽으면 그 행은 `CANCELING` 으로 남는다. 사용자 환불(`RefundFacade`)과 공통이고,
PG 취소가 `pgTxId` 기준 멱등이라 **재시도해도 이중 환불은 되지 않는다** — 재시도를 걸 사람이 없을 뿐이다.

...
**기간** 2026-08-06 ~ 08-07
**기준 커밋** `e49f92a` (main)
**코드 변경** 없음. 이 노트를 제외하면 전부 읽기와 설명이었다.

새 세션에서 학습을 이어가기 위한 출발점이다.
2장과 3장은 이미 끝난 것이라 다시 설명하거나 확인할 필요가 없다.
4장부터가 다음에 할 일이다.

> **2026-08-10 갱신.** 이 노트를 쓴 뒤 [decisions.md](decisions.md) 16~19 번이 들어가면서
> 근거로 든 코드가 몇 군데 움직였다. 달라진 자리는 본문에 그때그때 표시했다.
...
| 근거 우선 | 일반론이 아니라 이 저장소의 커밋과 실제 코드로 설명한다. `파일:줄번호` 로 짚는다. |
...
- (B) `afterCommit` 에서 발행 → 유령 이벤트는 사라지지만 커밋~발행 사이 창이 남는다. 그 순간 프로세스가 죽으면 이벤트는 영영 사라진다.

Outbox 의 본질은 발행이 아니다. "아직 발행되지 않았다"는 사실을 비즈니스 데이터와 같은 커밋에 남기는 것이다. 그 위에서만 재시도·백오프·DEAD 회수·순서 보장·적체 관측이 성립한다.
...
사고 B의 근본 원인은 단위 불일치다. 순서 보장의 단위(`orderNo`)와 download 가 관리하는 상태의 단위(`memberId:productId`)가 다르다. 두 주문이 같은 문서 하나를 놓고 경합하는데 그 문서를 키로 하는 순서 보장은 어디에도 없다.

판별 기준은 하나다. 두 이벤트의 파티션 키가 같은가. 같으면 릴레이가 막고 다르면 소비 측이 막아야 한다.
...
- 토픽은 논리적 이름이고 실제 데이터는 파티션에 나뉘어 저장된다. 이 프로젝트는 3개다(`KAFKA_NUM_PARTITIONS: 3`).
- 파티션은 추가만 되는 로그 파일이고 오프셋은 그 안에서의 줄 번호다. 파티션마다 따로 센다.
- 어느 파티션에 갈지는 키가 정한다. `hash(키) % 파티션수`. 같은 키는 언제나 같은 파티션이다.
...
### 2.4 컨슈머 그룹: "파티션 1개 = 컨슈머 1개" 는 그룹 내부 규칙이다
...
- 그룹 사이에는 제한이 없다. 100개 붙여도 다 읽는다.
- 그룹 안에서는 파티션 수가 컨슈머 수의 천장이다.

부수 정정 두 가지:

- "환불 토픽" 은 없다. 토픽은 애그리거트 단위이고 환불은 `stove.payment.v1` 의 `PaymentCancelled` 이벤트다.
- download 는 payment 토픽을 보지 않는다. license 토픽 구독자다. 환불 경로는 동시 소비가 아니라 연쇄(`payment → license → download`)다.
...
### 2.5 파티션 수는 병렬 처리량의 상한이다

비유하자면 파티션은 줄(lane)이고 컨슈머는 직원이다. 한 줄은 직원 한 명만 맡는다.
...
한 줄에 직원 둘을 붙일 수 없는 이유는 성능 타협이 아니라 순서와 오프셋이 동시에 깨지기 때문이다.

컨슈머가 줄면 리밸런스로 남은 컨슈머가 자동 인수한다. 그래서 "직원이 부족해 방치되는 줄" 은 구조적으로 생기지 않는다. 느려질 뿐 멈추지 않는다.

### 2.6 파티션 편중: 빈 줄은 사고가 아니다

- 빈 파티션이 생겨도 잃는 것은 "그 줄을 맡은 컨슈머가 논다" 뿐이다. 유실과 순서 문제는 없다.
- 편중 여부는 손님 수가 아니라 키의 종류 수(카디널리티)가 정한다. 경험칙으로 키 종류 ≥ 파티션 수 × 10 이면 안전권이다.
- 이 프로젝트: `orderNo` 계열 7개 이벤트는 무한 증가라 안전하다. `productCode` 계열 5개는 게임 수가 적은 초기에 편중되지만 그 시점엔 트래픽도 적어 무해하다.
- 진짜 위험은 반대 방향인 hot partition 이다. 빈 줄은 "직원을 덜 뽑으면 되는" 문제이고 몰린 줄은 "직원을 더 뽑아도 안 되는" 문제다.
...
그리고 [3]은 이 저장소에서 ArchUnit 으로 강제하고 있다 (`common/archunit/EventOrderingRules.java`):
...
성격은 서비스 중단이 아니라 이벤트 전달 지연이다. 실측 릴레이 처리량 480.5 events/s 기준으로, 5분간 초당 50건이 쌓였어도 재개 후 약 31초면 배수된다.

> 이 절차가 가능한 이유가 곧 outbox 를 쓰는 이유다. 동기 발행이었다면 발행을 멈추려면 결제 API 를 멈춰야 하고 안 멈추면 그동안의 이벤트가 사라진다.

**방법 B. 새 토픽 (`stove.payment.v2`)**. `Topics.java` 의 `v1` 접미사가 이걸 대비한 것이다. 무지연이지만 컨슈머 배포 2회와 두 토픽 운영으로 복잡하다.
...
## 3. 코드로 확인한 사실 (2026-08-07 확인, 2026-08-10 재대조 결과 전 항목 그대로)

새 세션에서 다시 확인하지 않아도 된다. 단 코드가 바뀌면 무효이므로 이상하면 재확인할 것.

| 확인한 것 | 결과 |
|---|---|
| 릴레이의 실행 형태 | 별도 프로세스가 아니다. 각 앱 안의 `@Scheduled` 빈 (`MessagingAutoConfiguration:58`) |
...
| 토픽 명시 생성 | 없다. `NewTopic`/`KafkaAdmin` 빈 부재 → auto-create 의존 |
| retention | 미설정이라 브로커 기본 7일 |
| `concurrency` | 전 서비스 미설정이라 리스너당 스레드 1개. 3배 여유가 통째로 남아 있음 |
| `auto-offset-reset` | 9개 서비스 전부 `earliest` (유실 회피 방향) |
| `ack-mode` | `record`. 한 건 처리마다 커밋한다 |
| `@KafkaListener` 개수 | download 3, payment 2, 나머지 7개 서비스 각 1 (총 12) |
| 리스너의 비동기 오프로드 | 없다. 그리고 ArchUnit 이 강제하고 있다 |
| `relay-enabled: false` | settlement 하나. "이벤트를 발행하지 않는 종단 컨슈머" 라 의도된 것 |
| 릴레이 다중화 | 불가하다. 서비스당 1대 제약 (`FOR UPDATE SKIP LOCKED` 가 파티션 키를 모름) |
...
| 순서 | 대상 | 왜 이 순서인가 |
|---|---|---|
| 1 | `common/messaging/inbox/` + `docs/decisions.md` 8절 | Outbox 의 짝이다. at-least-once 를 고른 대가를 치르는 곳. `(event_id, consumer_group)` 유니크. 그림이 닫힌다 |
...
> 학습 대상으로 더 값이 큰 것은 그 자리를 대신한 Outbox ↔ 분산 추적의 충돌이라 그쪽으로 바꿨다.

1번과 2번은 Kafka 구조가 머릿속에 올라와 있는 지금 보는 게 효율이 가장 좋다.
...
**① `EntitlementService.grant` 에는 `belongsTo` 가드가 없다** (관찰. 2026-08-10 재확인 결과 그대로다)
...
| `common/kafka/` | 수신 측 정책. `ConsumerRetryPolicy`, DLT, DLT 운영 API 가 있고 JPA 를 모른다 (19번) |
...
| `docs/decisions.md` | 설계 결정 23건, 각각 근거와 대가까지 |
...
> 19번이 수신 측 정책만 `common/kafka/` 로 갈라낸 이유는 정책만 쓰려는 store 와 download 에
